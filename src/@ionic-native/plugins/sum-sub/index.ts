...
@Plugin({
  pluginName: 'SumSub',
  plugin: 'cordova-sum-sub', // npm package name, example: cordova-plugin-camera
...
export class SumSub extends IonicNativePlugin {
  ...
  @Cordova()
  launchSNSMobileSDK(apiUrl: string, flowName: string, accessToken: string, locale: string, supportEmail: string): Promise<any> {
